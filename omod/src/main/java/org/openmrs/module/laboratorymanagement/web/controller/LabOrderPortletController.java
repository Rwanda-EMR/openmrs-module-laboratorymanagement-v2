...
public class LabOrderPortletController extends PortletController {
	...
	protected void populateModel(HttpServletRequest request, Map<String, Object> model) {
		...
		try {
			...
			List<Order> orders = LabUtils.getLabOrdersByPatient(patient);
			...
			System.out.println("Lab order portlet controller took: "+ Math.round((double)(t2-t1)/1000) + " seconds to load");

			super.populateModel(request, model);
		}
		catch (Throwable t) {
			log.error("An error occurred in the lab order portlet controller", t);
